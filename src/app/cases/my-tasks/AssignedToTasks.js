...
import { Clock, Calendar, AlertCircle, User, Eye, RefreshCw, Search, Filter, Briefcase } from 'lucide-react';
...
const AssignedToTasks = () => {
  ...
  const { data: response, error, isLoading, mutate } = useSWR(
    userId ? [`/tasks/assigned-to/${userId}`, userId, status, debouncedSearch, page] : null,
    () => getAssignedToTasks(userId, { 
              ...
              <Table>
                <TableHeader className="bg-muted/20">
                  <TableRow>
                    ...
                    <TableHead className={isRTL ? 'text-right' : 'text-left'}>{t('tasks.assignedBy')}</TableHead>
