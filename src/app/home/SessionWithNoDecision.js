...
function SessionWithNoDecision() {
    ...
    if (isLoading) {
        return (
            <Card className="flex flex-col h-full bg-white dark:bg-gray-900 border-none shadow-sm hover:shadow-md transition-shadow">
                <CardHeader className="bg-gradient-to-r from-orange-50/50 to-orange-100/50 dark:from-orange-900/10 dark:to-orange-800/10 border-b border-orange-100 dark:border-orange-900/30 pb-4">
                    <CardTitle className="text-orange-900 dark:text-orange-100 text-lg flex items-center justify-between">
    ...
    return (
        <Card className="flex flex-col h-full bg-white dark:bg-gray-900 border-none shadow-sm hover:shadow-md transition-shadow overflow-hidden">
            <CardHeader className="bg-gradient-to-r from-orange-50/50 to-orange-100/50 dark:from-orange-900/10 dark:to-orange-800/10 border-b border-orange-100 dark:border-orange-900/30 pb-4">
                <CardTitle className="text-orange-900 dark:text-orange-100 text-lg flex items-center justify-between">
                    <span>{t('home.delayedDecisions')}</span>
                    {sessions.length > 0 && (
                        <span 
                            className="relative inline-flex items-center justify-center px-2.5 py-0.5 text-xs font-medium bg-orange-500 text-white rounded-full"
                            aria-label={`${sessions.length} ${t('home.delayedDecisions')}`}
                        >
                            <span className="absolute inset-0 rounded-full bg-orange-500 animate-ping opacity-75" aria-hidden="true"></span>
                            <span className="relative" aria-hidden="true">{sessions.length}</span>
                        </span>
                    )}
                </CardTitle>
            </CardHeader>
